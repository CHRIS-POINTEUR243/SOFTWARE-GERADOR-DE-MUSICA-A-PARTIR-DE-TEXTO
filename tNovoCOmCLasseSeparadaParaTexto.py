...
class GeradorMusical:
    def __init__(self,texto,instrumento="ACOUSTIC_GRAND_PIANO",oitava=OITAVA_DEFAULT,volume=VOLUME_DEFAULT,bpm=BPM_DEFAULT):
        ...
        parcer=Parcer(self.juncaoDasTabelas,texto)
        self.listaCaracteres=parcer.processaTextoEmLista()
...
# TESTES
class Parcer:
    def __init__(self,tabela,texto):
        self.tabela=tabela
        self.texto=texto
        self.listaCaracteres=[]
        self.processaTextoEmLista()
            ...
            while i < tamanhoTexto:
                ...
                for tamanhoString in range(tamanhoTexto, 0, -1):
                    if (i + tamanhoString <= len(self.texto)) and (self.texto[i:i+tamanhoString] in self.tabela):
                        self.listaCaracteres.append(self.texto[i:i+tamanhoString])
                        i += tamanhoString
                        encontrou = True
                        break
